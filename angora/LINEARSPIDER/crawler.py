...
class ProxyManager(object):
    ...
    def load_pxy(self, replace = False):
        ...
        if replace: # if replace = True, delete current proxy before loading
            self.proxy = df
        else: # if replace = False, merge current proxy with loaded proxy from self.file_path
            try:
